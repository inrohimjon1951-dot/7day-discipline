...
export default function DashboardClient({
  ...
  const { time, date, countdown } = useTimer()
  ...
  const handleTaskChange = useCallback((payload: RealtimePostgresChangesPayload<Record<string, unknown>>) => {
    if (payload.eventType === 'DELETE') {
      const old = payload.old as TaskLog
      setLogs(prev => prev.filter(l => l.id !== old?.id))
    } else {
      const record = payload.new as TaskLog
      if (!record) return
      setLogs(prev => {
  ...
  const handleReactionChange = useCallback((_payload: RealtimePostgresChangesPayload<Record<string, unknown>>) => {
    // reactions refreshed inside UserPanel
  }, [])
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'streaks' }, (payload) => {
        const record = payload.new as Streak
        if (!record) return
        setStreaks(prev => {
  ...
  const myUser = allUsers.find(u => u.id === currentUser.id) || currentUser
  const otherUser = allUsers.find(u => u.id !== currentUser.id)

  const isShavkatjon = currentUser.username === 'shavkatjon'

  const leftUser = isShavkatjon ? otherUser : myUser
  const rightUser = isShavkatjon ? myUser : otherUser

  const panelData = [
    leftUser ? { user: leftUser, isMe: leftUser.id === currentUser.id } : null,
    rightUser ? { user: rightUser, isMe: rightUser.id === currentUser.id } : null,
  ].filter(Boolean) as { user: User; isMe: boolean }[]
    ...
    <div style={{ minHeight: '100vh', background: '#0a0a0f', fontFamily: "'Space Grotesk', sans-serif" }}>
      ...
      <div style={{
        ...
        <a href="/dashboard" style={{
          ...
          color: '#00f5ff', borderBottom: '2px solid #00f5ff', textDecoration: 'none',
        }}>Dashboard</a>
      ...
      <div style={{
        ...
        minHeight: 'calc(100vh - 110px)', position: 'relative',
      }}>
        <div style={{
        ...
        <div style={{
          ...
          fontSize: 9, fontWeight: 700, color: '#555570', letterSpacing: 1, zIndex: 2,
        }}>VS</div>

        {panelData.map(({ user, isMe }) => {
          const userLogs = logs.filter(l => l.user_id === user.id)
          const userStreak = streaks.find(s => s.user_id === user.id)
          const userHistory = history.filter(l => l.user_id === user.id)
          const col: 'cyan' | 'red' = user.username === 'muhammadyusuf' ? 'cyan' : 'red'
